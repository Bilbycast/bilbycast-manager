...
3. SRT - Receive RTP over SRT (Secure Reliable Transport)
   ...
   - Advanced optional: max_bw (max bandwidth bytes/s, 0=unlimited), input_bw (estimated input rate bytes/s, 0=auto), overhead_bw (overhead %, 5-100, default 25), enforced_encryption (bool, reject unencrypted peers), connect_timeout_secs (default 3), flight_flag_size (flow window pkts, default 25600), send_buffer_size/recv_buffer_size (buffer pkts, default 8192), payload_size (bytes per SRT packet, default 1316), ip_tos (DSCP 0-255), retransmit_algo ("default" or "reduced"), send_drop_delay (ms, -1=off), loss_max_ttl (reorder tolerance, 0=adaptive), km_refresh_rate/km_pre_announce (key rotation packets), mss (max segment size 76-9000, default 1500, adjust for VPN/jumbo frames), tlpkt_drop (bool, too-late packet drop, default true, disable for recording), ip_ttl (IP TTL 1-255, default 64)
